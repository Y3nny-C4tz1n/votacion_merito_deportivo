...
class ResultadoeController extends MY_Controller {
	public function __construct(){
		parent::__construct();
		$this->load->model('ResultadoeModel');
	...
	public function index(){
		...
		$datos = $this->funcionesBasicas('Resultado Entrenador');
		$datos['menu'] = "rentrenador";
		$datos['MODALIDAD'] = $this->CatalogoModel->listModalidad();
		$this->load->view('resultado-entrenador',$datos);
	}
	public function listRentrenador(){
		...
		$count = $this->ResultadoeModel->ResultadoeModelo($filtros);
		...
		$filtros['sidx'] = $sidx;
		$filtros['sord'] = $sord;
		$dataControl = $this->ResultadoeModel->ResultadoeModelo($filtros);
		if($count > 0){
